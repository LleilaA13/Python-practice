...
def check_list(res, expected, params=None, expl=''):
    if res != expected:
        if isinstance(res, list):
            ...
            my_print('Here the two lists')
            if diff>=0:
                expected = expected + [""]*diff
                zz = zip(res, expected)
                my_print('{:<30}   {:>30}'.format('RETURNED', 'EXPECTED'))
            else:
                res = res+[""]*(-diff)
                zz = zip(expected, res)
                my_print('{:<30}   {:>30}'.format('EXPECTED', 'RETURNED'))
            for i, j in zz:
                if i!=j:
                    my_print(f'\b[ERROR]{i:<30}   {j:>30}')
                else:
                    my_print(f'{i:<30}   {j:>30}')
        else:
