...
def func1(int_list, n):
    ...
    pass
    return { v: int_list.count(v) for v in int_list if int_list.count(v) >= n }
...
def func2(dict1, a, b):
    ...
    def criterio (el):
        return -len(el),el
    return sorted( [dict1[k] for k in dict1 if a.lower()<=k[0].lower()<=b.lower()], key=criterio)
...
def func3(str1, str2):
    ## Scrivi qui il tuo codice
    pass
    return ''.join(c for a,b in zip(str1,reversed(str2)) for c in [a.lower() if a.isupper() else a.upper(), b])
...
def ex1(dirin):
    ...
    pass
    D = {}
    esplora_dir(dirin, 0, D)
    return sorted(D, key=lambda x: (-D[x][0], D[x][1], x))

def esplora_dir(dirin, profondita, D):
    for nome in os.listdir(dirin):
        path = dirin + '/' + nome
        if os.path.isfile(path) and nome.endswith('.txt'):
            with open(path, encoding='utf8') as FIN:
                testo = FIN.read()
                digits = [c for c in testo if c.isdigit()]
                if digits:
                    D[path] = (len(digits), profondita)
        elif os.path.isdir(path):
            esplora_dir(path, profondita+1, D)
