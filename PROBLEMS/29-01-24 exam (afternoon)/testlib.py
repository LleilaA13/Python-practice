...
def check_list(res, expected, params=None, expl=''):
    if res != expected:
        if isinstance(res, list):
            ...
            for i, j in zz:
                if i!=j:
                    print(f'\b[ERROR]{str(i):<30}   {str(j):>30}')
                else:
                    print(f'{str(i):<30}   {str(j):>30}')
        else:
...
def check_dict(res, expected, params=None, expl=''):
    if res != expected:
        if isinstance(res, dict):
            ...
            for k in expected:
                if k not in res:
                    print(f'''\b[ERROR] Manca la chiave {k} con valore {expected[k]}!''')
                    print(f'''\b[ERROR] Key {k} with value {expected[k]} is missing!''')
                elif expected[k] != res[k]:
                        print(f'''\b[ERROR] La chiave {k} dovrebbe avere il valore {expected[k]} invece che {res[k]}''')
                        print(f'''\b[ERROR] Key {k} should have the value {expected[k]} instead of {res[k]}''')
            diff = set(res.keys()) - set(expected.keys())
            if diff:
                for extra_key in diff:
                    print(f'''\b[ERROR] La chiave {extra_key} è di troppo.''')
                    print(f'''\b[ERROR] La key {extra_key} is unexpected.''')
        else:
