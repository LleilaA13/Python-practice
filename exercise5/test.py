...
class Test(testlib.TestCase):
    ...
    def do_test(self, lista, expected, expectedLista):
        '''Test implementation
            - list : list of integers and strings
            - expected : expected dictionary
            - expectedList : resulting list of integers
        '''
        with self.ignored_function('builtins.print'), \
             self.forbidden_function('os.walk'):
             #self.timer(2):
            result = program.ex54(lista)
        ...
        self.assertEqual(
            lista, expectedLista, f"La lista deve essere modificata in {expectedLista} invece che {lista}/Input list should become {expectedLista} instead of {lista}")
